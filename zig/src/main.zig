...
//             .buffer = try gpa_.alloc(u8, buffer_size),
//             .stations = std.StringHashMap(StationSummary).init(gpa_),
//             .thread = undefined,
//         };
...
//             std.mem.copyForwards(u8, selected_worker.?.*.buffer, previous_buffer[previous_buffer_size - copy_from_previous ..]);
//         }
